...
public class TestSuiteSpecifications implements Specification<TestSuiteEntity>
{
	
	private static final long serialVersionUID = 1L;
	public List<SearchCriteria> testSuiteSearchCriteriaList;
	
	public TestSuiteSpecifications()
	{
		testSuiteSearchCriteriaList=new ArrayList<SearchCriteria>();
	}
	
	public void addNewTestSuiteSpecifications(SearchCriteria newSearchCriteria)
	{
		testSuiteSearchCriteriaList.add(newSearchCriteria);
	}
	
	@Override
	public Predicate toPredicate(Root<TestSuiteEntity> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder)
	{
		List<Predicate> predicatesList = new ArrayList<Predicate>();
		
		for (SearchCriteria searchCriteria : testSuiteSearchCriteriaList)
		{
...
//						}
	...
	public List<Predicate> conditions(List<Predicate> predicatesList,SearchCriteria searchCriteria, String fieldValue, Root<TestSuiteEntity> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder)
	{
		if(searchCriteria.getOperator().equals(SearchOperator.IN))
		{
	...
	public List<Predicate> conditionsNumber(List<Predicate> predicatesList,SearchCriteria searchCriteria, long fieldValue, Root<TestSuiteEntity> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder)
	{
		if(searchCriteria.getOperator().equals(SearchOperator.IN))
		{
			predicatesList.add(criteriaBuilder.in(root.get(searchCriteria.getKey())).value(fieldValue));
